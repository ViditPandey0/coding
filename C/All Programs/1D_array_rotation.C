...
void rotateToRight(int *arr,int l,int n){
    
    //Rotating the array to the right n number of times
    for(int i=0;i<n;i++){
        int temp=arr[l-1];
        for(int j=l-1;j>0;j--){
            arr[j]=arr[j-1];
    ...
    //Printing array elements after rotating it to the right n number of times 
    printf("The elements of the array after rotating it %d times to the right are : ",n);
    for(int i=0;i<l;i++){
        printf("%d ",arr[i]);
    }
...
int main(){
    ...
    printf("Enter 1 to rotate the array to the left.\nEnter 2 to rotate the array to the right.\nEnter your choice : ");
    int choice;
    scanf("%d",&choice);
